...
def test_model():
    ...
    for model_name in model_name_list:
        # load data
        with open('./tmp_pkl_data/{}_whole_year_gt.pkl'.format(building_name), 'rb') as r:
    ...
    # output as csv file
    with open('./test_result/test_on_{}_gt_result.csv'.format(building_name), 'w', newline='') as f:
        write = csv.writer(f, dialect=('excel'))
        write.writerow(['rank', 'model_name', 'MAE'])
        for index, model in enumerate(sorted_models):
            write.writerow([index + 1, model[0], model[1]])
    print('result saved at: ./test_result/test_on_{}_gt_result.csv'.format(building_name))
