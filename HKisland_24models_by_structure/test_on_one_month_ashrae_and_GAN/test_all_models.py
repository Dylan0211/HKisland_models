...
from HKisland_24models_by_structure.model import *
...
def test_model():
    ...
    # output as csv file
    with open('./test_result/test_on_{}_with_{}_GAN_ashrae_{}_to_{}_result.csv'.format(
            building_name, GAN_building_name, source_context, target_context), 'w', newline='') as f:
        write = csv.writer(f, dialect=('excel'))
        write.writerow(['rank', 'model_name', 'MAE'])
        for index, model in enumerate(sorted_models):
            write.writerow([index + 1, model[0], model[1]])
    print('result saved at: ./test_result/test_on_{}_with_{}_GAN_ashrae_{}_to_{}_result.csv'.format(
        building_name, GAN_building_name, source_context, target_context))
